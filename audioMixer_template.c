...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	pthread_mutex_lock(&audioMutex);
	{
		...
		// Exit if fail
		//fprintf(stderr, "ERROR: Too many sounds to play at the same time.\n");
		//exit(EXIT_FAILURE);
	}
...
// Sum 2 bites with overflow/underflow prevention
static short sumBites(short bite1, short bite2)
{
	short sum = bite1 + bite2;
	
	// Underflow
	if (bite1 < 0 && bite2 < 0 && sum > 0) {
		sum = SHRT_MIN;
	}
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	for (int playbackBufferIndex = 0; playbackBufferIndex < size; playbackBufferIndex++) {
		short sum = 0;
		// Add all sound bites to 1 buffer slot
		for (int i = 0; i < MAX_SOUND_BITES; i++) {
			pthread_mutex_lock(&audioMutex);
			{
				if ((soundBites[i]).pSound != NULL) {
					...
					short nextBite = (((soundBites[i]).pSound)->pData)[offset];

					// Add sound bite to sum
					sum = sumBites(sum, nextBite);
		...
		playbackBuffer[playbackBufferIndex] = sum;
	}
}
